...
| **Value** | `AI_META_VALUE_RE` | A value is *content*. Naming a vendor in it says nothing about how the file was made. Only provenance-shaped phrasing counts: `ai-generated`, a watermark scheme (`synthid`, `c2pa`), or an explicit `generated with <vendor>`. |
...
Narrowing the value pattern removed the only thing that caught keys like `generated-with:` and
`made_with:`, whose *values* are bare vendor names. `AI_META_NAME_RE` therefore also matches
provenance phrases — `(generated|created|made|written|produced|authored)[-_ ]?(with|by|using)` — so
those keys fire on the key, where they always should have.
